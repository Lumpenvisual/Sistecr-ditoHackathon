...
describe("CrediPass", function () {
  ...
  async function deployFixture() {
    ...
    const cCOP = await MockCCOP.deploy(owner.address);
    ...
    return { owner, user, registrar, other, cCOP, creditNFT, rewardSystem };
  }
  ...
  describe("MockCCOP - faucet", function () {
    it("cualquier usuario puede reclamar 1000 mcCOP del faucet", async function () {
      const { cCOP, user } = await loadFixture(deployFixture);

      await cCOP.connect(user).faucet();
      expect(await cCOP.balanceOf(user.address)).to.equal(ethers.parseEther("1000"));
    });
  });
  ...
  describe("RewardSystem - integración completa", function () {
    async function fundedFixture() {
      ...
      const { cCOP, rewardSystem, owner } = ctx;

      // El owner deposita fondos para que el RewardSystem pueda pagar recompensas.
      const deposit = ethers.parseEther("100000");
      await cCOP.connect(owner).approve(await rewardSystem.getAddress(), deposit);
      await rewardSystem.connect(owner).depositFunds(deposit);

      return ctx;
    }

    it("distribuye recompensas en mcCOP y actualiza la línea de crédito", async function () {
      const { cCOP, creditNFT, rewardSystem, owner, user } = await loadFixture(fundedFixture);
      ...
      // reward = base(0.5) + 360*0.001 (0.36) + base/2 por racha>=6 (0.25) = 1.11 mcCOP
      await rewardSystem.connect(owner).recordPayment(1);

      expect(await cCOP.balanceOf(user.address)).to.equal(ethers.parseEther("1.11"));
      expect(await rewardSystem.totalRewards(1)).to.equal(ethers.parseEther("1.11"));

      // límite = 100 + (360/100)*10 = 130 mcCOP
