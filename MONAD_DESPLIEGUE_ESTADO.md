...
Migración de CrediPass a **Monad L1** (EVM-compatible). Todo el código está
...
- **Frontend conectado a los contratos** (ya no es solo demo):
  - `frontend/src/lib/abis.ts` — ABIs de CreditNFT, RewardSystem, MockCCOP.
...
- **Tests de Hardhat**: `contracts/test/CrediPass.test.js` — 13/13 pasando.
...
3. **Conectar el frontend:** copia las 3 direcciones a `frontend/.env.local`:
   ...
   NEXT_PUBLIC_MONAD_CCOP_CONTRACT=0x...
